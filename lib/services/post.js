...
exports = module.exports = function(
  app, storage, db, factory, yaml, validator, socket, author)
{
  ...
  var collection = db.collection(storage.resource('post', {
    ...
    pipeline: [yaml, meta, validate, assign, process]
  }));
  ...
  collection.on('document-added', function(ev) {
    eventEmitter.emit('post-added', ev.object);
  });
  collection.on('document-changed', function(ev) {
    eventEmitter.emit('post-changed', ev.object);
  });
  collection.on('document-removed', function(ev) {
    eventEmitter.emit('post-removed', ev.object);
  });
  collection.on('document-error', function(ev) {
    eventEmitter.emit('post-error', ev.err);
  });
