...
exports = module.exports = function(app, storage, cache, factory) {
  ...
  function load(name) {
    ...
    if(post.status === 'published') {
      factory.process(post, function(result) {
        if(cache.put('post', name, result)) {
          eventEmitter.emit('post-added', result);
        } else {
          eventEmitter.emit('post-changed', result);
        }
      });
      return post;
    } else {
   ...
   * @param {Function} fn - Callback delivering the post.
   ...
   * Get all posts.
   *
   * @param {Function} fn - Callback delivering a list of posts.
   */
  ...
  this.findRelated = function(post, other) {
    var posts = other || cache.list('post');
  ...
  storage.on('post-removed', remove);
