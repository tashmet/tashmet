...
function transformData(func, loaders, data, absPath, query) {
  for(var i=0; i<loaders.length; i++) {
    if(func == 'load') {
      data = loaders[i].load(data, absPath, query);
    } else if(func == 'serialize') {
      data = loaders[i].serialize(data, absPath, query);
    }
  }
  return data;
}
...
Resource.prototype.get = function(query, cb) {
  ...
  getPath(query, this.params, this.pattern, cb, function(absPath) {
    fs.readFile(absPath, 'utf-8', function(err, data) {
      if(!err) {
        try {
          data = transformData('load', loaders, data, absPath, query);
        } catch(e) {
          err = new ResourceError(query, e.message);
        }
      }
      cb(err, data, absPath);
    });
...
Resource.prototype.store = function(query, data, cb) {
  ...
  getPath(query, this.params, this.pattern, cb, function(absPath) {
    data = transformData('serialize', loaders, data, absPath, query);
