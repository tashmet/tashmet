...
function input(pipeline, data, absPath, query, cb) {
  async.eachSeries(pipeline, function(pipe, done) {
    pipe.input(data, absPath, query, function(err, result) {
      data = result;
      done(err);
    });
  }, function(err) {
...
function output(pipeline, data, absPath, query, cb) {
  async.eachSeries(pipeline.reverse(), function(pipe, done) {
    pipe.output(data, absPath, query, function(err, result) {
      data = result;
      done(err);
    });
  }, function(err) {
  ...
  getPath(query, this.params, this.pattern, cb, function(absPath) {
    fs.readFile(absPath, 'utf-8', function(err, data) {
      read(err, data, function(err, data) {
        if(!err) {
          input(pipeline, data, absPath, query, function(err, data) {
...
Resource.prototype.store = function(query, data, cb) {
  ...
  getPath(query, this.params, this.pattern, cb, function(absPath) {
    output(pipeline, data, absPath, query, function(err, data) {
      if(!err) {
