 ...
 * @listens post-error
 */
exports = module.exports = function(log, socket, post, taxonomy, author, content) {
  ...
  post.on('post-error', function(ev) {
    log('ERR', 'post: ' + chalk.grey(ev.query.name));
    console.log('\n    ' + chalk.red(ev) + '\n');
  });
  ...
  author.on('author-added', function(author) {
    log('ADD', 'author: ' + chalk.grey(author.name));
  });
  author.on('author-changed', function(author) {
    log('UPD', 'author: ' + chalk.grey(author.name));
  });
  author.on('author-removed', function(author) {
    log('REM', 'author: ' + chalk.grey(author.name));
  });
  author.on('author-error', function(ev) {
    log('ERR', 'author: ' + chalk.grey(ev.query.name));
    console.log('\n    ' + chalk.red(ev) + '\n');
  });
