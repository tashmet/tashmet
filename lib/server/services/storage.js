 ...
 * @class Resource
 */
function Resource(name, def) {
  var identityPipe = {
    run: function(data, options, cb) {
      cb(null, data);
    }
  };
  ...
  this.input = def.input || identityPipe;
  this.output = def.output || identityPipe;
  this.read = def.read || function(err, data, cb) { cb(err, data); };
}
...
Resource.prototype.get = function(query, cb) {
  var read = this.read;
  var pipeline = this.input;

  getPath(query, this.params, this.pattern, cb, function(absPath) {
    fs.readFile(absPath, 'utf-8', function(err, data) {
      read(err, data, function(err, data) {
        if(!err) {
          pipeline.run(data, {path: absPath, query: query}, function(err, result) {
            if(!err) {
              for(var key in query) {
                result['__' + key] = query[key];
              }
            }
            cb(err, result, absPath);
          });
        } else {
...
Resource.prototype.store = function(query, data, cb) {
  var pipeline = this.output;

  getPath(query, this.params, this.pattern, cb, function(absPath) {
    pipeline.run(data, {path: absPath, query: query}, function(err, result) {
      if(!err) {
        for(var key in query) {
          delete result['__' + key];
        }
        fs.writeFile(absPath, result, function(err) {
...
function FileCollection(resource) {
  var collection = this;

  resource.on('resource-added', function(ev) {
    collection.get(ev.id, function(err, obj) {
  ...
  this.store = function(obj, cb) {
    resource.store({id: obj.__id}, obj, cb);
  };
}

util.inherits(FileCollection, events.EventEmitter);
...
exports = module.exports = function() {
  ...
  this.resource = function(name, definition) {
    if(definition) {
      resources[name] = new Resource(name, definition);
    }
    return resources[name];
  };


  this.directory = function(name, options) {
    var resOptions = {
      pattern: name + '/{id}.' + options.extension,
      input: options.input,
      output: options.output
    };

    return new FileCollection(storage.resource(name, resOptions));
  };
